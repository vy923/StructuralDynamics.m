...
for f = flags
    ...
    case -Inf 
        assert( ~any(T(:,3)==-Inf & T(:,2)==0), ...
                "tab: zero PSD and -Inf slope" )
    case Inf
        assert( ~any(T(:,4)==Inf & T(:,2)~=0 & ~isnan(T(:,2))), ...
                "tab: nonzero PSD and +Inf slope" )
    case 1
        assert( ~any(W2==W1 & any([LS RS]~=0 & maskNNS, 2) & W2~=0), ...
                "tab: nonzero slope in constant W band" )
    case 2
        assert( ~any(W2~=W1 & ~any(maskNB,2) & (LS==0 | RS==0)), ...
                "tab: zero slope in nonconstant W band" )
    case 3 
        assert( ~any(sum([LS RS].*maskNNS, 2)), ...
                "tab: inconsistent W left and right slopes" )
    case 4 
        assert( ~any([W1 W2]==0 & [RS LS]~=Inf & maskNNS(:,[2 1]) & [W2 W1]~=0 & ~maskNB, 'all'), ...
                "tab: zero W and noninf/nonnan slope -> nonzero/nonnan W" )
    case 5
        assert( ~any(all([W1 W2]==0,2) &  any(maskNNS & [LS RS]~=0, 2)), ...
                "tab: zero W1 and W2 with noninf/nonnan slope" )
    otherwise
        continue
    end
end